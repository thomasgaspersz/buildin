...
const PartnerRequestList = ({requests}:{requests: RequestWithUser[]}) => {
  ...
  return (
    <div className=''>
      ...
      <div className='flex items-center gap-4'>
        ...
        <span className='font-semibold flex-1'>{request.sender.name && request.sender.surname ? request.sender.name + ' ' + request.sender.surname : request.sender.username}</span>
        {request.sender.business && <span className='flex-1 font-light text-xs'>{request.sender.business}</span>}
        </div>
      </div>
